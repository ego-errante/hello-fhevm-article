...
**7. Putting the UI together: The Root Layout (`app/layout.tsx`)**
...
**2. The Entry Point: The Home Page (`app/page.tsx`)**
...
**3. The Main Event: A Guided Build of `RockPaperScissorsDemo.tsx`**

We will now build our primary interactive component, `packages/site/components/RockPaperScissorsDemo.tsx`, piece by piece.

**3.1. The Component Skeleton and Hook Connection**

Let's create the file and add this code:
...
Inside our component, we first call `useMetaMask()` to check for wallet connectivity and `useFhevm()` to get our FHEVM instance object. We then pass all of this information into our main orchestrator hook, `useRockPaperScissors`.

With all this setup, our component now has access to everything it needs to display game state, handle user actions, and show results. In the next steps, we'll wire these up to our JSX.
